...
* Add optional data, depending on the result file, e.g.: `add_sub()`.
* Filter result object by some criteria, e.g.: `filter_by(...)`.
...
JACUSA2helper uses `attributes()` to store meta data.

```{r}
names(attributes(rdd))
```

`attributes(rdd)$header` and `attributes(rdd)$method_type` are JACUSA2 specific fields.

```{r}
# attributes(rdd)$header
attributes(rdd)$header
# attributes(rdd)$method_type
attributes(rdd)$method_type
```

Use dplyr to manipulate the result object `rdd`. Check details on [dplyr](https://github.com/tidyverse/dplyr) and read `vignette("dplyr")`. Unfortunately, dplyr will lose JACUSA2 specific meta data. JACUSA2helper functions that use dplyr internally, will automatically preserve the JACUSA2 specific meta data with `copy_attr()`. Use `copy_attr()` at the end of a analysis pipeline to preserve attributes.

Once the result object is available, you can start to manipulate it. In a first step, we will apply a filter to the score.

```{r}
# before filtering
dim(rdd)
result <- filter_by_min_score(rdd, 2)
# after filtering
dim(result)
```

Filtering by the score (*call*) or pvalue (*rt-arrest* and *lrt-arrest*) columns does not require grouping by site because each data sample of site a will have the same score or pvalue.

dplyr and `%>%` from magrittr allow to formulate compact analysis pipelines by chaining functions with `%>%`. In the following, we will:
...
filtered <- rdd %>% 
    ...
    group_by_site() %>% # following function will be apply to site(s)
    filter_by(all(cov >= 10) & base_count(bc, ref) <= 2) %>%
    filter_by_robust_variants() %>%
    dplyr::ungroup() %>% # remove grouping
    redefine_ref(ref_cond = 1) %>% # use condition 1 to define reference
    add_sub() # add base substitutions
```
...
result$data_desc <- ""
result$data_desc[result$cond == 1] <- "DNA"
# add repl(licate) info
result$data_desc[result$cond == 2] <- paste0("RNA ", result$repl[result$cond == 2])
...
* emirical cumulative distribution of arrest rate or
...
SIIIRTMg. The result of JACUSA2 are availble by using `data()`. Read [JACUSA2 manual] for details on how data has been processed from the original publication by @Zhou2018. In brief, the data has been filtered to contain the following rRNAs: RNA18SN5 and RNA28SN5.
...
* Columns prefixed with `bc_`, `cov_` will hold observed bases and coverage for the respective base counts, respectively.
...
We investigate the strand specific coverage distribution of read of both rRNA and discover that more reads are mapped to the "+" strand.
...
filtered <- result %>%
  ...
  group_by_site() %>% # following function will be applied on site(s)
  filter_by(all(cov >= 100)) %>%
...
When working with stranded RNA-Seq data, inverting base calls is not necessary because JACUSA2 will automatically invert Single End (SE) and Paired End (PE) depending on the provided library type option "-P" UNSTRANDED|FR_FIRSTSTRAND|RF_SECOND_STRAND".
...
By default variables stored in the **info** column will be processed and unpacked making them available for further manipulation. Use `read_result(..., unpack = FALSE)` to prevent processing of the **info*** column. This will make reading output much faster.
...
* `filter_by()` Wrapper for `dplyr::filter()` that preserves JACUSA2 specific meta data in `attributes()`.
