...
* Read JACUSA2 output file(s) into result object - currently represented as `GenomicRanges::GRanges`.
...
* Filter result object by some criteria, e.g.: `dplyr::filter(All(cov$cond1 >= 10))` - 
  retain sites with read coverage >= 10, and use `All()` or `Any` on columns with data for conditions and replicates.
...
The main data structure is the result object that is currently represented as `GenomicRanges::GRanges`.
See [An Introduction to the GenomicRanges Package](https://bioconductor.org/packages/devel/bioc/vignettes/GenomicRanges/inst/doc/GenomicRangesIntroduction.html).

In brief, JACUSA2 specific data is stored as metadata and can be accessed via `GenomicRanges::mcols`.

[plyranges](https://www.bioconductor.org/packages/release/bioc/html/plyranges.html) enables to use `GenomicRanges` within [dplyr](https://github.com/tidyverse/dplyr) and [ggplot2](https://github.com/tidyverse/ggplot2).
...
comparisons, read `vignette("Introduction to meta conditions with JACUSA2helper")`. 
Expect increased memory requirements when using multiple JACUSA2 output with `read_results`.
Make sure all JACUSA2 output files have the same number of conditions and replicates when using `read_results`.

Reading files serially with `read_result` is much more memory efficient and allows for file specific filtering.
...
Note that `rdd` is a `GenomicRanges:GRanges` object - check [An Introduction to the GenomicRanges Package](https://bioconductor.org/packages/devel/bioc/vignettes/GenomicRanges/inst/doc/GenomicRangesIntroduction.html) for 
more details. 
...
[dplyr](https://github.com/tidyverse/dplyr) and `%>%` from [magrittr](https://cran.r-project.org/web/packages/magrittr/vignettes/magrittr.html) allow to formulate compact analysis pipelines. 
...
# calculate how many different base calls are present at site across conditions and replicates
GenomicRanges::mcols(filtered)["bc"] <- lapply_cond(filtered$bases, function(b) { Reduce("+", b) } ) %>% 
  Reduce("+", .) %>% base_count()
...
Robust requires observations (here base calls) to be present in all replicates of at least one condition.
...
```{r}
HIVRT_rt_arrest %>% select(cov) %>% 
  mutate(total_cov = lapply_cond(cov, rowSums) %>% rowSums) %>% 
  select(-cov) %>% 
  as.data.frame() %>%
  ggplot2::ggplot(ggplot2::aes(x = total_cov, colour = strand)) + 
...
```{r}
filtered %>% select(arrest_rate) %>%
  mutate(arrest_rate1 = arrest_rate$cond1$rep1, arrest_rate2 = arrest_rate$cond2$rep1) %>%
  select(-arrest_rate) %>%
  as.data.frame() %>%
