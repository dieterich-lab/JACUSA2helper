...
```{r}
filtered <- rdd
GenomicRanges::mcols(filtered)["bc"] <- lapply_cond(filtered$bases, function(b) { Reduce("+", b) } ) %>% Reduce("+", .) %>% base_count()
filtered %>%
...
```{r}
#gather_repl(HIVRT_rt_arrest$cov) %>% # extract coverages
...
<!-- TODO
```{r}
#GenomicRanges::mcols(filtered) %>% as.data.frame() %>%
