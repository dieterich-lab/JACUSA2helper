...
Given a JACUSA2 result object, the function `create_data()` will create a data matrix, that will be used in NMF. 
...
The learning part can be omitted. 
Use `data(m6a_nmf_results)` from @Piechotta2021 and and continue to [Visualize NMF].
...
We will predict modification sites only on WT vs KO from the previous example.
If you have your own data, follow the steps above until `create_data()`.
Here, we will modify the existing `data_matrix`:
