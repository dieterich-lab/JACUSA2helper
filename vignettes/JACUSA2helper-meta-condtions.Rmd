...
```{r}
filtered_meta <- Zhou2018_rt_arrest %>% 
...
data(rnam)
rnam <- rnam %>% 
  dplyr::filter(rrna == "18S", base == "psU")

Zhou2018_rt_arrest %>%
  dplyr::filter(contig == "18S") %>%
  dplyr::filter(pvalue <= 0.1) %>%
  dplyr::group_by(meta_cond) %>%
  dplyr::filter(All(cov$cond1 >= 10) & All(cov$cond1 >= 10)) %>%
  ...
  ggplot2::ggplot(ggplot2::aes(x = end, y = arrest_rate_delta, fill = meta_cond)) +
    ...
    ggplot2::geom_point(data = rnam, ggplot2::aes(x = pos, y = 0), fill = "black") +
    ggplot2::xlab("Position on 18S") +
